...
import kz.mounty.fm.domain.requests.{GetCurrentUserRoomsRequestBody, GetRoomAndRoomTracksRequestBody, GetRoomsForExploreRequestBody}
  ...
  def roomRoutes(implicit ex: ExecutionContext,
    ...
    get {
      parameters(
        ...
        ctx =>
          val body = write(GetCurrentUserRoomsRequestBody(limit, offset, tokenKey))
          completeRequest(publisher, write(body), RoomCore.GetCurrentUserRoomsRequest.routingKey, exchange, ctx)
      }
    }
  }
